/**
 * Semantic Scholar 数据源 Fetcher 封装。
 */

import type { Article } from '../types';
import type { Fetcher, SearchOptions } from './base-fetcher';
import { getCached, setCached, makeCacheKey } from '../cache';
import { searchSemanticScholar as apiSearch, getPaperById as apiGetById } from '../semantic-scholar';

export class SemanticScholarFetcher implements Fetcher {
  name = 'Semantic Scholar';
  sourceType = 'paper' as const;

  async search(options: SearchOptions): Promise<Article[]> {
    const { query, limit = 20, offset = 0 } = options;
    const cacheKey = makeCacheKey('ss', { q: query, l: limit, o: offset });
  ...
  async fetchById(id: string): Promise<Article | null> {
    const cacheKey = `ss-byid-${id}`;
    const cached = getCached<Article>(cacheKey);
    if (cached) return cached;

    const article = await apiGetById(id.replace('ss-', ''));
